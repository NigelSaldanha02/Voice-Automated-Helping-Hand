...
# Convert class names to lowercase list
class_list = []
for name in classes.values():
    class_list.append(name.lower())
...
while True:
    ...
    for result in results:
        ...
        for box in boxes:
            ...
            # Class name
            cls = int(box.cls[0])
            detected_name = classes[cls].lower()

            # Only process the object spoken by the user
            if detected_name != keyword[0]:
                continue
            ...
            print(f"{detected_name}: pixel=({cx},{cy})  world=({x_real:.1f}cm, {y_real:.1f}cm)")
